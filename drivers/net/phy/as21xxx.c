...
static int enable_rate_adaptation = 1;
module_param(enable_rate_adaptation, int, 0644);
MODULE_PARM_DESC(enable_rate_adaptation, "Enable rate adaptation [on/off]");
...
static void as21xxx_read_speed(struct phy_device *phydev, int bmcr)
{
	int speed;
...
static void as21xxx_resolve_aneg_linkmode(struct phy_device *phydev)
{
...
static int as21xxx_read_status(struct phy_device *phydev)
{
	...
	if (phydev->autoneg == AUTONEG_ENABLE && phydev->autoneg_complete) {
		as21xxx_read_speed(phydev, bmcr);
		as21xxx_resolve_aneg_linkmode(phydev);
